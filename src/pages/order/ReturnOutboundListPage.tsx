...
/**
 * 반품 출고 목록 — OutboundOrder 중 originType='return' 만 표시.
 * 행 클릭 시 일반 OutboundDetailPage 로 이동 (반품 컨텍스트는 거기서 표시).
 */
export default function ReturnOutboundListPage() {
  const navigate = useNavigate();
  const { data: orders = [], isLoading } = useOutboundOrders({ originType: 'return' });
  ...
  const columns: ColumnsType<OutboundOrder> = [
    ...
    {
      title: '원본 입고지시서', dataIndex: 'return_from_order_no', key: 'return_from_order_no', width: 160,
      render: (v?: string | null) => v || <Text type="secondary">-</Text>,
    },
    {
      title: '입고처(반품 대상)', key: 'supplier_name', width: 160,
      render: (_, r) => <span>↩ {r.supplier_name ?? '-'}</span>,
    },
  ...
  return (
    <div className="order-list-tone" style={{ color: '#334155', fontSize: 14, lineHeight: 1.4 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <Space>
          <Title level={4} style={{ margin: 0 }}>반품 출고 목록</Title>
          <Text type="secondary" style={{ fontSize: 12 }}>입고처에 다시 보낸 출고지시서</Text>
        </Space>
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={() => navigate('/order/return-outbound')}
        >
          반품 출고 접수
        </Button>
      </div>
      ...
      {orders.length === 0 && !isLoading ? (
        <Empty
          ...
          description={
            <div>
              <div style={{ fontSize: 14, color: '#1e2a3a', marginBottom: 4 }}>아직 반품 출고가 없습니다</div>
              <div style={{ fontSize: 12, color: '#6b7280' }}>입고처에 클레임 발생 시 여기에 표시됩니다.</div>
            </div>
          }
          style={{ marginTop: 80 }}
        >
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/order/return-outbound')}>
            반품 출고 접수
          </Button>
        </Empty>
      ) : (
        <Table
          ...
          size="middle"
          onRow={(r) => ({
            onClick: () => navigate(`/order/outbound/${r.id}`),
